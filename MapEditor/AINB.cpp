    ...
    if (this->EXBFile.Loaded) {
        ...
        for (EXB::CommandInfoStruct Command : this->EXBFile.Commands) {
            ...
            for (EXB::CommandInfoStruct Child : this->Functions) {
                if (Child.BaseIndexPreCommandEntry == Command.BaseIndexPreCommandEntry && Child.InstructionCount == Command.InstructionCount && Child.InputDataType == Command.InputDataType && Child.StaticMemorySize == Command.StaticMemorySize) {
                    FoundCmd = true;
                    break;
                }
            }
    ...
    for (int i = 0; i < Header.NodeCount; i++) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int j = 0; j < Count; j++) {
                ...
                if (LocalImmediateParameters[i].size() <= (Index + j))
                    continue;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int j = 0; j < Count; j++) {
                ...
                if (LocalInputParameters[i].size() <= (Index + j))
                    continue;
...
std::vector<unsigned char> AINBFile::ToBinary()
{
    ...
    if (this->Header.FileCategory == "Logic")
        FileCategoryNum = 1;
    if (this->Header.FileCategory == "Sequence")
        FileCategoryNum = 2;
            ...
            for (AINBFile::InputEntry& Entry : Node.InputParameters[Type]) {
                if (Entry.Function.InstructionCount > 0 && Entry.Function.Instructions.size() > 0) {
                    ...
                    for (int i = 0; i < EXBCommands.size(); i++) {
                        if (EXBCommands[i].Instructions.size() != Entry.Function.Instructions.size() || EXBCommands[i].InputDataType != Entry.Function.InputDataType || EXBCommands[i].OutputDataType != Entry.Function.OutputDataType || EXBCommands[i].Scratch32MemorySize != Entry.Function.Scratch32MemorySize || EXBCommands[i].Scratch64MemorySize != Entry.Function.Scratch64MemorySize || EXBCommands[i].StaticMemorySize != Entry.Function.StaticMemorySize || EXBCommands[i].BaseIndexPreCommandEntry != Entry.Function.BaseIndexPreCommandEntry)
                            continue;

                        for (int j = 0; j < EXBCommands[i].Instructions.size(); j++) {
                            if (EXBCommands[i].Instructions[j].DataType != Entry.Function.Instructions[j].DataType || EXBCommands[i].Instructions[j].LHSIndexValue != Entry.Function.Instructions[j].LHSIndexValue || EXBCommands[i].Instructions[j].LHSSource != Entry.Function.Instructions[j].LHSSource || EXBCommands[i].Instructions[j].RHSIndexValue != Entry.Function.Instructions[j].RHSIndexValue || EXBCommands[i].Instructions[j].RHSSource != Entry.Function.Instructions[j].RHSSource || EXBCommands[i].Instructions[j].Signature != Entry.Function.Instructions[j].Signature || EXBCommands[i].Instructions[j].StaticMemoryIndex != Entry.Function.Instructions[j].StaticMemoryIndex || EXBCommands[i].Instructions[j].Type != Entry.Function.Instructions[j].Type)
                                continue;
            ...
            for (int Type = 0; Type < AINBFile::LinkedNodeTypeCount; Type++) {
                if (Type == (int)AINBFile::LinkedNodeMapping::OutputBoolInputFloatInputLink) {
                    for (AINBFile::LinkedNodeInfo Entry : Node.LinkedNodes[Type]) {
                        ...
                        bool IsInput = false;
                        if (Node.Type == (uint16_t)AINBFile::NodeTypes::Element_Expression || Node.Type == (uint16_t)AINBFile::NodeTypes::Element_S32Selector || Node.Type == (uint16_t)AINBFile::NodeTypes::Element_F32Selector || Node.Type == (uint16_t)AINBFile::NodeTypes::Element_StringSelector || Node.Type == (uint16_t)AINBFile::NodeTypes::Element_RandomSelector || Node.Type == (uint16_t)AINBFile::NodeTypes::Element_BoolSelector) {
                            for (int i = 0; i < AINBFile::ValueTypeCount; i++) {
                                for (AINBFile::InputEntry Param : Node.InputParameters[i]) {
                    ...
                    for (AINBFile::LinkedNodeInfo Entry : Node.LinkedNodes[Type]) {
                        if (Node.Type == (int)AINBFile::NodeTypes::Element_F32Selector) {
                            ...
                            Writer.Seek(Pos, BinaryVectorWriter::Position::Begin);
                            Current += 40;
                        } else if (Node.Type == (int)AINBFile::NodeTypes::Element_StringSelector || Node.Type == (int)AINBFile::NodeTypes::Element_S32Selector || Node.Type == (int)AINBFile::NodeTypes::Element_RandomSelector) {
                            Writer.WriteInteger(Current, sizeof(uint32_t));
                            int Pos = Writer.GetPosition();
                            Writer.Seek(Current, BinaryVectorWriter::Position::Begin);
                            Writer.WriteInteger(Entry.NodeIndex, sizeof(uint32_t));
    ...
    for (int i = 0; i < AINBFile::ValueTypeCount; i++) {
        if (!InputParameters[i].empty())
            InputParametersEmpty = false;
        if (!OutputParameters[i].empty())
            OutputParametersEmpty = false;
    }
    ...
    for (AINBFile::Node& Node : this->Nodes) {
        if (std::find(Node.Flags.begin(), Node.Flags.end(), AINBFile::FlagsStruct::IsExternalAINB) != Node.Flags.end()) {
            ...
            if (!Initial)
                EmbeddedAINBs[Node.GetName()] = EmbeddedAINBs[Node.GetName()] + 1;
            else
                EmbeddedAINBs.insert({ Node.GetName(), 1 });
        }
    }
    for (auto& [Key, Val] : EmbeddedAINBs) {
