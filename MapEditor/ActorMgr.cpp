...
{
    if (!Node.HasChild("Gyaml"))
        return nullptr;

    Actor& BymlActor = ModifyActor == nullptr ? AddActor(Node.GetChild("Gyaml")->GetValue<std::string>(), false) : *ModifyActor;

    if (Node.HasChild("Hash"))
        BymlActor.Hash = Node.GetChild("Hash")->GetValue<uint64_t>();
    if (Node.HasChild("SRTHash"))
        BymlActor.SRTHash = Node.GetChild("SRTHash")->GetValue<uint32_t>();

    if (Node.HasChild("Translate"))
        BymlActor.Translate = Node.GetChild("Translate")->GetValue<Vector3F>();
    ...
    if (Node.HasChild("Scale"))
        BymlActor.Scale = Node.GetChild("Scale")->GetValue<Vector3F>();

    if (Node.HasChild("Bakeable"))
        BymlActor.Bakeable = Node.GetChild("Bakeable")->GetValue<bool>();
    if (Node.HasChild("IsPhysicsStable"))
        BymlActor.PhysicsStable = Node.GetChild("IsPhysicsStable")->GetValue<bool>();
    if (Node.HasChild("MoveRadius"))
        BymlActor.MoveRadius = Node.GetChild("MoveRadius")->GetValue<float>();
    if (Node.HasChild("ExtraCreateRadius"))
        BymlActor.ExtraCreateRadius = Node.GetChild("ExtraCreateRadius")->GetValue<float>();
    if (Node.HasChild("IsForceActive"))
        BymlActor.ForceActive = Node.GetChild("IsForceActive")->GetValue<bool>();
    if (Node.HasChild("IsInWater"))
        BymlActor.InWater = Node.GetChild("IsInWater")->GetValue<bool>();
    if (Node.HasChild("TurnActorNearEnemy"))
        BymlActor.TurnActorNearEnemy = Node.GetChild("TurnActorNearEnemy")->GetValue<bool>();
    if (Node.HasChild("Name"))
        BymlActor.Name = Node.GetChild("Name")->GetValue<std::string>();
    ...
    if (Node.HasChild("Links")) {
        for (BymlFile::Node& LinksChild : Node.GetChild("Links")->GetChildren()) {
            ...
            for (BymlFile::Node& LinkNode : LinksChild.GetChildren()) {
                if (LinkNode.GetKey() == "Dst")
                    Link.Dest = LinkNode.GetValue<uint64_t>();
                if (LinkNode.GetKey() == "Gyaml")
                    Link.Gyml = LinkNode.GetValue<std::string>();
                if (LinkNode.GetKey() == "Name")
                    Link.Name = LinkNode.GetValue<std::string>();
                if (LinkNode.GetKey() == "Src")
                    Link.Src = LinkNode.GetValue<uint64_t>();
            }
    ...
    if (Node.HasChild("Rails")) {
        for (BymlFile::Node& RailsChild : Node.GetChild("Rails")->GetChildren()) {
            ...
            for (BymlFile::Node& RailNode : RailsChild.GetChildren()) {
                if (RailNode.GetKey() == "Dst")
                    Rail.Dest = RailNode.GetValue<uint64_t>();
                if (RailNode.GetKey() == "Gyaml")
                    Rail.Gyml = RailNode.GetValue<std::string>();
                if (RailNode.GetKey() == "Name")
                    Rail.Name = RailNode.GetValue<std::string>();
            }
    ...
    if (BymlActor.Dynamic.DynamicString.count("BancPath")) {
        ...
        for (BymlFile::Node& ActorNode : File.GetNode("Actors")->GetChildren()) {
            ...
            float NewX = BymlActor.Translate.GetX() + ((MergedActor.Translate.GetX() - BymlActor.Translate.GetX()) * (std::cosf(RadianRotate.GetZ()) * std::cosf(RadianRotate.GetY()))) + ((MergedActor.Translate.GetY() - BymlActor.Translate.GetY()) * (std::cosf(RadianRotate.GetZ()) * std::sinf(RadianRotate.GetY()) * std::sinf(RadianRotate.GetX()) - std::sinf(RadianRotate.GetZ()) * std::cosf(RadianRotate.GetX()))) + ((MergedActor.Translate.GetZ() - BymlActor.Translate.GetZ()) * (std::cosf(RadianRotate.GetZ()) * std::sinf(RadianRotate.GetY()) * std::cosf(RadianRotate.GetX()) + std::sinf(RadianRotate.GetZ()) * std::sinf(RadianRotate.GetX())));

            float NewY = BymlActor.Translate.GetY() + ((MergedActor.Translate.GetX() - BymlActor.Translate.GetX()) * (std::sinf(RadianRotate.GetZ()) * std::cosf(RadianRotate.GetY()))) + ((MergedActor.Translate.GetY() - BymlActor.Translate.GetY()) * (std::sinf(RadianRotate.GetZ()) * std::sinf(RadianRotate.GetY()) * std::sinf(RadianRotate.GetX()) + std::cosf(RadianRotate.GetZ()) * std::cosf(RadianRotate.GetX()))) + ((MergedActor.Translate.GetZ() - BymlActor.Translate.GetZ()) * (std::sinf(RadianRotate.GetZ()) * std::sinf(RadianRotate.GetY()) * std::cosf(RadianRotate.GetX()) - std::cosf(RadianRotate.GetZ()) * std::sinf(RadianRotate.GetX())));

            float NewZ = BymlActor.Translate.GetZ() + ((MergedActor.Translate.GetX() - BymlActor.Translate.GetX()) * (-std::sinf(RadianRotate.GetY()))) + ((MergedActor.Translate.GetY() - BymlActor.Translate.GetY()) * (std::cosf(RadianRotate.GetY()) * std::sinf(RadianRotate.GetX()))) + ((MergedActor.Translate.GetZ() - BymlActor.Translate.GetZ()) * (std::cosf(RadianRotate.GetY()) * std::cosf(RadianRotate.GetX())));
...
BymlFile::Node ActorMgr::ActorToByml(Actor& ExportedActor)
{
    ...
    */
    ...
    if (!ExportedActor.Dynamic.DynamicString.empty() || !ExportedActor.Dynamic.DynamicVector.empty()) {
        ...
        for (const auto& [Key, Value] : ExportedActor.Dynamic.DynamicString) {
            BymlFile::Node DynamicEntry(GetDynamicValueDataType(Value), Key);
            if (DynamicEntry.GetType() == BymlFile::Type::UInt32)
                DynamicEntry.GetType() = BymlFile::Type::Int32;
        ...
        for (const auto& [Key, Value] : ExportedActor.Dynamic.DynamicVector) {
    ...
    if (!ExportedActor.Phive.Placement.empty() || !ExportedActor.Phive.ConstraintLink.Owners.empty() || ExportedActor.Phive.ConstraintLink.ID != 0 || !ExportedActor.Phive.ConstraintLink.Refers.empty()) {
            ...
            if (!ExportedActor.Phive.ConstraintLink.Owners.empty()) {
                ...
                for (Actor::PhiveData::ConstraintLinkData::OwnerData& Owner : ExportedActor.Phive.ConstraintLink.Owners) {
                    ...
                    if (!Owner.BreakableData.empty()) {
                        ...
                        for (const auto& [Key, Value] : Owner.BreakableData) {
                    ...
                    if (!Owner.ClusterData.empty()) {
                        ...
                        for (const auto& [Key, Value] : Owner.ClusterData) {
                    ...
                    if (Owner.OwnerPose.Rotate.GetX() != 0 || Owner.OwnerPose.Rotate.GetY() != 0 || Owner.OwnerPose.Rotate.GetZ() != 0 || Owner.OwnerPose.Translate.GetX() != 0 || Owner.OwnerPose.Translate.GetY() != 0 || Owner.OwnerPose.Translate.GetZ() != 0) {
                    ...
                    if (Owner.ReferPose.Rotate.GetX() != 0 || Owner.ReferPose.Rotate.GetY() != 0 || Owner.ReferPose.Rotate.GetZ() != 0 || Owner.ReferPose.Translate.GetX() != 0 || Owner.ReferPose.Translate.GetY() != 0 || Owner.ReferPose.Translate.GetZ() != 0) {
                    ...
                    if (!Owner.UserData.empty()) {
                        ...
                        for (const auto& [Key, Value] : Owner.UserData) {
        ...
        if (!ExportedActor.Phive.Placement.empty()) {
            ...
            for (const auto& [Key, Value] : ExportedActor.Phive.Placement) {
    ...
    if (!ExportedActor.Presence.empty()) {
        ...
        for (const auto& [Key, Value] : ExportedActor.Presence) {
...
void ActorMgr::DeleteActor(uint64_t Hash, uint32_t SRTHash)
{
            ...
            for (Actor& Child : ActorMerged.MergedActorContent) {
                if (Child.Hash == Hash && Child.SRTHash == SRTHash) {
                    ActorMerged.MergedActorContent.erase(
                        std::remove_if(ActorMerged.MergedActorContent.begin(), ActorMerged.MergedActorContent.end(), [&](const Actor& Actor) {
    ...
    Actors.erase(
        std::remove_if(Actors.begin(), Actors.end(), [&](const Actor& Actor) {
