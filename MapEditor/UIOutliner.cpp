...
void UIOutliner::SelectActor(Actor* pActor)
{
    ...
    if (pActor != nullptr)
        ImGuizmo::RecomposeMatrixFromComponents(pActor->Translate.GetRawData(), pActor->Rotate.GetRawData(), pActor->Scale.GetRawData(), UIMapView::ObjectMatrix);
}
...
void UIOutliner::DrawOutlinerWindow()
{
    ...
    if (ImGui::BeginListBox("##Outliner1", ImVec2(ImGui::GetWindowSize().x - ImGui::GetStyle().FramePadding.x, ImGui::GetWindowHeight() - 48))) {
        for (int i = 0; i < ActorNames.size(); i++) {
            if (ActorNames[i].Gyml.find(FilterText) == std::string::npos && FilterText.length() > 0)
                continue;
