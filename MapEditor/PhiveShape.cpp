...
int64_t PhiveShape::ReadPackedInt(BinaryVectorReader& Reader)
{
    ...
    case 0x1C:
        return ((FirstByte << 24) | (Reader.ReadUInt8() << 16) | (Reader.ReadUInt8() << 8) | Reader.ReadUInt8()) & 0x7FFFFFF;

    case 0x1D:
        return ((FirstByte << 32) | (Reader.ReadUInt8() << 24) | (Reader.ReadUInt8() << 16) | (Reader.ReadUInt8() << 8) | Reader.ReadUInt8()) & 0x7FFFFFFFFFFFFFF;

    case 0x1E:
        return ((FirstByte << 56) | (Reader.ReadUInt8() << 48) | (Reader.ReadUInt8() << 40) | (Reader.ReadUInt8() << 32) | (Reader.ReadUInt8() << 24) | (Reader.ReadUInt8() << 16) | (Reader.ReadUInt8() << 8) | Reader.ReadUInt8()) & 0x7FFFFFFFFFFFFFF;

    case 0x1F:
        return (FirstByte & 7) == 0
            ? ((FirstByte << 40) | (Reader.ReadUInt8() << 32) | (Reader.ReadUInt8() << 24) | (Reader.ReadUInt8() << 16) | (Reader.ReadUInt8() << 8) | Reader.ReadUInt8()) & 0xFFFFFFFFFF
            : (FirstByte & 7) == 1
            ? (Reader.ReadUInt8() << 56) | (Reader.ReadUInt8() << 48) | (Reader.ReadUInt8() << 40) | (Reader.ReadUInt8() << 32) | (Reader.ReadUInt8() << 24) | (Reader.ReadUInt8() << 16) | (Reader.ReadUInt8() << 8) | Reader.ReadUInt8()
            : 0;
...
void PhiveShape::hkRelArrayView<T>::Write(BinaryVectorWriter& Writer, int32_t Offset)
{
    ...
    if (this->BaseOffset > 0)
        Writer.Seek(this->BaseOffset, BinaryVectorWriter::Position::Begin);
...
std::vector<unsigned char> PhiveShape::ToBinary(std::vector<float> Vertices, std::vector<unsigned int> Indices)
{
    ...
    for (int i = 0; i < Indices.size() / 3; i++) {
        ...
        for (int j = 0; j < Indices.size() / 3; j++) {
            ...
            uint32_t Matching = 0;
            if (SubPointA == PointA || SubPointA == PointB || SubPointA == PointC)
                Matching++;
            else
                DifferentIndex = Indices[j * 3];
            if (SubPointB == PointA || SubPointB == PointB || SubPointB == PointC)
                Matching++;
            else
                DifferentIndex = Indices[j * 3 + 1];
            if (SubPointC == PointA || SubPointC == PointB || SubPointC == PointC)
                Matching++;
            else
                DifferentIndex = Indices[j * 3 + 2];
        ...
        for (std::vector<uint32_t> Indice : WrittenIndices) {
            if (std::find(Indice.begin(), Indice.end(), Primitive.IdA) != Indice.end() && std::find(Indice.begin(), Indice.end(), Primitive.IdB) != Indice.end() && std::find(Indice.begin(), Indice.end(), Primitive.IdC) != Indice.end() && std::find(Indice.begin(), Indice.end(), Primitive.IdD) != Indice.end()) {
                Found = true;
                std::cout << "FOUND\n";
                break;
            }
    ...
    for (int k = 0; k < MeshShape.GeometrySections.Elements.back().Primitives.Elements.size(); k++) {
        for (int i = 0; i < MeshShape.GeometrySections.Elements.back().Primitives.Elements.size(); i++) {
            if (std::find(SortedPrimitives.begin(), SortedPrimitives.end(), i) != SortedPrimitives.end())
                continue;
            ...
            if (LowestPoint.GetY() > PointB.GetY())
                LowestPoint = PointB;
            if (LowestPoint.GetY() > PointC.GetY())
                LowestPoint = PointC;
            if (LowestPoint.GetY() > PointD.GetY())
                LowestPoint = PointD;

            if (PointA.GetY() == LowestPoint.GetY())
                LowestPointCount++;
            if (PointB.GetY() == LowestPoint.GetY())
                LowestPointCount++;
            if (PointC.GetY() == LowestPoint.GetY())
                LowestPointCount++;
            if (PointD.GetY() == LowestPoint.GetY())
                LowestPointCount++;
            ...
            for (int j = 0; j < MeshShape.GeometrySections.Elements.back().Primitives.Elements.size(); j++) {
                if (j == i)
                    continue;
                if (std::find(SortedPrimitives.begin(), SortedPrimitives.end(), j) != SortedPrimitives.end())
                    continue;
                ...
                if (ChildLowestPoint.GetY() > ChildPointB.GetY())
                    ChildLowestPoint = ChildPointB;
                if (ChildLowestPoint.GetY() > ChildPointC.GetY())
                    ChildLowestPoint = ChildPointC;
                if (ChildLowestPoint.GetY() > ChildPointD.GetY())
                    ChildLowestPoint = ChildPointD;

                if (ChildPointA.GetY() == ChildLowestPoint.GetY())
                    ChildLowestPointCount++;
                if (ChildPointB.GetY() == ChildLowestPoint.GetY())
                    ChildLowestPointCount++;
                if (ChildPointC.GetY() == ChildLowestPoint.GetY())
                    ChildLowestPointCount++;
                if (ChildPointD.GetY() == ChildLowestPoint.GetY())
                    ChildLowestPointCount++;
    ...
    */
    ...
    HktSize = Util::ByteswapU32(HktSize);
    ...
    Writer.WriteInteger(Util::ByteswapU32(DataSize), sizeof(uint32_t));
    Writer.Seek(-4, BinaryVectorWriter::Position::Current);
    Writer.WriteByte(0x40);
