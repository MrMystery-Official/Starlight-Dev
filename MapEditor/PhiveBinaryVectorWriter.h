...
class PhiveBinaryVectorWriter : public BinaryVectorWriter
{
public:
	...
	void WriteHkPointer(T* ObjectPointer, uint32_t ObjectSize, std::string ClassName, std::string PointerName)
	{
		...
		if constexpr (std::is_base_of_v<hkWriteable, T>) {
			...
			{
				Logger::Error("PhiveBinaryVectorWriter", "Could not write pointer");
			}
		}
		else
		{
		...
		std::cout << "Wrote pointer\n";
	}

	PhiveWrapper& mPhiveWrapper;

	PhiveBinaryVectorWriter(PhiveWrapper& mWrapper);
};