...
/* --- Node --- */
BymlFile::Node::Node(BymlFile::Type Type, std::string Key)
    : m_Type(Type)
    , m_Key(Key)
{
}
...
template <>
void BymlFile::Node::SetValue(std::string Value) // Value is a string
{
    this->m_Value.clear();
...
template <class T>
void BymlFile::Node::SetValue(T Value) // Value is a default C++ type
{
    this->m_Value.clear();
...
{
    return m_Key == Comp.m_Key && m_Value == Comp.m_Value && m_Type == Comp.m_Type && m_Children == Comp.m_Children;
}
...
std::vector<unsigned char> BymlFile::ToBinary(BymlFile::TableGeneration TableGeneration)
{
    ...
    Logger::Info("BymlEncoder", "Encoded BYML data, took " + std::to_string((duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()) - StartMS).count()) + "ms");
