...
void AINBNodeDefMgr::DecodeAINB(std::vector<unsigned char> Bytes, std::string FileName, std::vector<EXB::CommandInfoStruct>* EXBCommands)
{
    ...
    if (!File.Loaded || File.Nodes.empty())
        return;
    AINBNodeDefMgr::NodeDef::CategoryEnum FileCategory = AINBNodeDefMgr::NodeDef::CategoryEnum::Logic;
    if (File.Header.FileCategory == "AI")
        FileCategory = AINBNodeDefMgr::NodeDef::CategoryEnum::AI;
    else if (File.Header.FileCategory == "Sequence")
        FileCategory = AINBNodeDefMgr::NodeDef::CategoryEnum::Sequence;
    for (AINBFile::Node Node : File.Nodes) {
        ...
        for (AINBNodeDefMgr::NodeDef& Definition : AINBNodeDefMgr::NodeDefinitions) {
            if (Definition.DisplayName == Node.GetName()) {
                NodeFound = true;
                if (Definition.FileNames.size() < 10)
                    Definition.FileNames.push_back(FileName);
                // Definition.FileNames.push_back(FileName);
                if (std::find(Definition.AllowedAINBCategories.begin(), Definition.AllowedAINBCategories.end(), FileCategory) == Definition.AllowedAINBCategories.end())
                    Definition.AllowedAINBCategories.push_back(FileCategory);
