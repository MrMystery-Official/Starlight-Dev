...
void UIMSBTEditor::Initialize()
{
    ...
    for (const auto& DirEntry : recursive_directory_iterator(Editor::GetRomFSFile("Mals", false))) {
        if (DirEntry.is_regular_file() && DirEntry.path().filename().string().ends_with(".sarc.zs")) {
            ...
            if (DirEntry.path().filename().string().starts_with("CNzh"))
                Lang = UIMSBTEditor::Language::CNzh;
            if (DirEntry.path().filename().string().starts_with("EUde"))
                Lang = UIMSBTEditor::Language::EUde;
            if (DirEntry.path().filename().string().starts_with("EUen"))
                Lang = UIMSBTEditor::Language::EUen;
            if (DirEntry.path().filename().string().starts_with("EUes"))
                Lang = UIMSBTEditor::Language::EUes;
            if (DirEntry.path().filename().string().starts_with("EUfr"))
                Lang = UIMSBTEditor::Language::EUfr;
            if (DirEntry.path().filename().string().starts_with("EUit"))
                Lang = UIMSBTEditor::Language::EUit;
            if (DirEntry.path().filename().string().starts_with("EUnl"))
                Lang = UIMSBTEditor::Language::EUnl;
            if (DirEntry.path().filename().string().starts_with("EUru"))
                Lang = UIMSBTEditor::Language::EUru;
            if (DirEntry.path().filename().string().starts_with("JPja"))
                Lang = UIMSBTEditor::Language::JPja;
            if (DirEntry.path().filename().string().starts_with("KRko"))
                Lang = UIMSBTEditor::Language::KRko;
            if (DirEntry.path().filename().string().starts_with("TWzh"))
                Lang = UIMSBTEditor::Language::TWzh;
            if (DirEntry.path().filename().string().starts_with("USen"))
                Lang = UIMSBTEditor::Language::USen;
            if (DirEntry.path().filename().string().starts_with("USes"))
                Lang = UIMSBTEditor::Language::USes;
            if (DirEntry.path().filename().string().starts_with("USfr"))
                Lang = UIMSBTEditor::Language::USfr;
