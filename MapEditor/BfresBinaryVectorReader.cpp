...
{
	BfresFile::BfresAttribFormat Format = (BfresFile::BfresAttribFormat)RawFormat;
	switch (Format)
	{
...
{
	if (StringCache.count(Offset))
	{
		return StringCache[Offset];
	}
...
std::vector<bool> BfresBinaryVectorReader::ReadBooleanBits(uint32_t Count)
{
	...
	for (int i = 0; i < Count; i++)
	{
		...
		Bools.push_back((BitFlags[Idx] & ((long)1 << i)) != 0);
	}
