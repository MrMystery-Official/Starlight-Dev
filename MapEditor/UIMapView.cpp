...
{
	if (!RenderSettings.AllowSelectingActor || !Focused) return;
...
void UIMapView::DrawMapViewWindow()
{
	...
	SelectActorByClicking(ImVec2(ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().y), ImVec2(ImGui::GetMousePos().x - ImGui::GetWindowPos().x - ImGui::GetWindowContentRegionMin().x,
