...
namespace application::file::game::ainb
{
	...
	int EXB::GetOffsetInStringTable(std::string Value, std::vector<std::string>& StringTable)
	{
		...
		if (Index == -1)
		{
			application::util::Logger::Warning("EXBDecoder", "Can not find string \"" + Value + "\" in StringTable");
			return -1;
		}
	...
	std::string EXB::ReadStringFromStringPool(application::util::BinaryVectorReader* Reader, uint32_t Offset) {
	...
	EXB::EXB(std::vector<unsigned char> Bytes)
	{
		...
		{
			application::util::Logger::Error("EXBDecoder", "Wrong magic, expected magic EXB");
			return;
		}
		if (Header.Version != 0x02)
		{
			application::util::Logger::Error("EXBDecoder", "Wrong version, expected 0x02");
			return;
		}

		//Signature Offsets
		Reader.Seek(Header.SignatureTableOffset, application::util::BinaryVectorReader::Position::Begin);
		...
		//Command Info
		Reader.Seek(Header.CommandInfoOffset, application::util::BinaryVectorReader::Position::Begin);
		...
		Reader.Seek(Header.CommandTableOffset, application::util::BinaryVectorReader::Position::Begin);
		...
		for (int i = 0; i < InstructionCount; i++)
		{
			...
			{
				Reader.Seek(7, application::util::BinaryVectorReader::Position::Current);
				...
				auto Func = [&Instruction, &Reader](std::string i, EXB* EXBFile)
					{
						if ((i == "LHS" ? Instruction.LHSSource : Instruction.RHSSource) == EXB::Source::ParamTbl)
						{
							...
							Reader.Seek(EXBFile->Header.ParameterRegionOffset + (i == "LHS" ? Instruction.LHSIndexValue : Instruction.RHSIndexValue), application::util::BinaryVectorReader::Position::Begin);
							...
							{
								(i == "LHS" ? Instruction.LHSValue : Instruction.RHSValue) = glm::vec3(Reader.ReadFloat(), Reader.ReadFloat(), Reader.ReadFloat());
							}
		...
		for (EXB::CommandInfoStruct& Command : Commands)
		{
			...
			{
				if(this->Instructions.size() > InstructionIndex + i)
	...
	std::vector<unsigned char> EXB::ToBinary(uint32_t EXBInstanceCount)
	{
		...
		Writer.Seek(36, application::util::BinaryVectorWriter::Position::Current);
		...
		for (EXB::InstructionStruct Instruction : NewInstructions)
		{
			...
			else
			{
				...
				Writer.Seek(7, application::util::BinaryVectorWriter::Position::Current);
			}
		}
		...
		for (EXB::InstructionStruct Instruction : NewInstructions)
		{
			if (*reinterpret_cast<uint32_t*>(&Instruction.LHSValue) != 0xFFFF)
			{
				...
				{
					Writer.Seek(ParamStart + Instruction.LHSIndexValue, application::util::BinaryVectorWriter::Position::Begin);
					if (Instruction.DataType == EXB::Type::Vec3f)
					{
						Writer.WriteRawUnsafeFixed(reinterpret_cast<const char*>(&reinterpret_cast<glm::vec3*>(&Instruction.LHSValue)->x), sizeof(float));
						Writer.WriteRawUnsafeFixed(reinterpret_cast<const char*>(&reinterpret_cast<glm::vec3*>(&Instruction.LHSValue)->y), sizeof(float));
						Writer.WriteRawUnsafeFixed(reinterpret_cast<const char*>(&reinterpret_cast<glm::vec3*>(&Instruction.LHSValue)->z), sizeof(float));
				...
				{
					Writer.Seek(ParamStart + Instruction.RHSIndexValue, application::util::BinaryVectorWriter::Position::Begin);
					if (Instruction.DataType == EXB::Type::Vec3f)
					{
						Writer.WriteRawUnsafeFixed(reinterpret_cast<const char*>(&reinterpret_cast<glm::vec3*>(&Instruction.RHSValue)->x), sizeof(float));
						Writer.WriteRawUnsafeFixed(reinterpret_cast<const char*>(&reinterpret_cast<glm::vec3*>(&Instruction.RHSValue)->y), sizeof(float));
						Writer.WriteRawUnsafeFixed(reinterpret_cast<const char*>(&reinterpret_cast<glm::vec3*>(&Instruction.RHSValue)->z), sizeof(float));
						StringStart = std::max(StringStart, (uint32_t)Writer.GetPosition());
					}
					else if (Instruction.DataType == EXB::Type::S32)
					{
		...
		Writer.Seek(StringStart, application::util::BinaryVectorWriter::Position::Begin);
		...
		Writer.Seek(8, application::util::BinaryVectorWriter::Position::Begin);
