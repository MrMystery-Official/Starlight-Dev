...
namespace application::file::game::ainb
{
	...
	std::string AINBFile::ReadStringFromStringPool(application::util::BinaryVectorReader* Reader, uint32_t Offset) {
		int BaseOffset = Reader->GetPosition();
		Reader->Seek(Header.StringOffset + Offset, application::util::BinaryVectorReader::Position::Begin);
		std::string Result;
		...
		Reader->Seek(BaseOffset, application::util::BinaryVectorReader::Position::Begin);
		return Result;
	}

	AINBFile::GUIDData AINBFile::ReadGUID(application::util::BinaryVectorReader* Reader) {
		...
		Reader->ReadStruct(reinterpret_cast<char*>(Data.Part5), 6);

		return Data;
	}

	AINBFile::AttachmentEntry AINBFile::ReadAttachmentEntry(application::util::BinaryVectorReader* Reader) {
	...
	AINBFile::InputEntry AINBFile::ReadInputEntry(application::util::BinaryVectorReader* Reader, int Type)
	{
	...
	AINBFile::OutputEntry AINBFile::ReadOutputEntry(application::util::BinaryVectorReader* Reader, int Type) {
	...
	std::pair<bool, size_t> FindContiguousSubsequence(
		const std::vector<uint16_t>& s,
		const std::vector<uint16_t>& v)
	{
		if (v.empty()) return { true, 0 };

		// Check if all elements exist and form a contiguous range
		auto it = std::find(s.begin(), s.end(), v.front());
		if (it == s.end()) return { false, 0 };

		size_t index = std::distance(s.begin(), it);
	...
	void AINBFile::Initialize(std::vector<unsigned char> Bytes, bool SkipErrors) {
		...
		char magic[4];
		Reader.ReadStruct(magic, 3);
		Reader.Seek(1, application::util::BinaryVectorReader::Position::Current);
		magic[3] = '\0';
		if (strcmp(magic, "AIB") != 0) {
			application::util::Logger::Error("AINBDecoder", "Wrong magic, expected AIB");
			return;
		}
		...
		if (Header.Version != 0x404 && Header.Version != 0x407) {
			application::util::Logger::Error("AINBDecoder", "Wrong version, expected 0x404(4.4) or 0x407(4.7, got " + std::to_string(Header.Version));
			return;
		}
		...
		*/
		...
		/* Global Parameters */
		Reader.Seek(Header.GlobalParameterOffset, application::util::BinaryVectorReader::Position::Begin);
		...
		for (int i = 0; i < 6; i++) {
			...
			Reader.Seek(2, application::util::BinaryVectorReader::Position::Current);
		...
		for (int i = 0; i < this->GlobalParameters.size(); i++) {
			...
			Reader.Seek(Pos + Type.Offset, application::util::BinaryVectorReader::Position::Begin);
			for (GlobalEntry& Entry : this->GlobalParameters[i])
			{
				...
				if (i == (int)AINBFile::GlobalType::Vec3f) { //vec3f
					Entry.GlobalValue = glm::vec3(Reader.ReadFloat(), Reader.ReadFloat(), Reader.ReadFloat());
				}
		...
		*/
		...
		if (Header.EXBOffset != 0) {
			Reader.Seek(Header.EXBOffset, application::util::BinaryVectorReader::Position::Begin);
		...
		Reader.Seek(Header.ImmediateOffset, application::util::BinaryVectorReader::Position::Begin);
		...
		for (int i = 0; i < this->ImmediateOffsets.size(); i++) {
			Reader.Seek(this->ImmediateOffsets[i], application::util::BinaryVectorReader::Position::Begin);
			if (i < 5) {
				while (Reader.GetPosition() < this->ImmediateOffsets[i + 1]) {
					...
					if (i == 4) { //vec3f
						...
						float Y = Reader.ReadFloat();
						float Z = Reader.ReadFloat();
						Parameter.Value = glm::vec3(X, Y, Z);
					}
				...
				while (Reader.GetPosition() < Header.IOOffset) {
					...
					if (i == 4) { //vec3f
						Parameter.Value = glm::vec3(Reader.ReadFloat(), Reader.ReadFloat(), Reader.ReadFloat());
					}
					Parameter.ValueType = i;
					this->ImmediateParameters[i].push_back(Parameter);
				}
		...
		if (Header.AttachmentCount >= 1) {
			Reader.Seek(Header.AttachmentOffset, application::util::BinaryVectorReader::Position::Begin);
			...
			for (AttachmentEntry& Param : this->AttachmentParameters) {
				...
				for (int i = 0; i < 6; i++) {
					...
					if (Index == 65535)
					{
						application::util::Logger::Error("AINBDecoder", "Attachment Index was -1");
						break;
					}
					uint32_t Count = Reader.ReadUInt32();
					if (Count > 1000)
					{
						if(!SkipErrors) application::util::Logger::Error("AINBDecoder", "Attachment Count was too big (>1000)");
						break;
					}
		...
		/* IO Parameters */
		Reader.Seek(Header.IOOffset, application::util::BinaryVectorReader::Position::Begin);
		for (int i = 0; i < 12; i++) {
			if (i % 2 == 0) {
		...
		for (int Type = 0; Type < this->InputParameters.size(); Type++) {
			for (InputEntry& Parameter : this->InputParameters[Type]) {
				if (Parameter.MultiIndex != -1) {
					Reader.Seek(Header.MultiOffset + Parameter.MultiIndex * 8, application::util::BinaryVectorReader::Position::Begin);
		...
		Reader.Seek(Header.ResidentUpdateOffset, application::util::BinaryVectorReader::Position::Begin);
		...
		/* Precondition Nodes */
		Reader.Seek(Header.PreconditionOffset, application::util::BinaryVectorReader::Position::Begin);
		...
		/* Entry String */
		Reader.Seek(Header.EntryStringOffset, application::util::BinaryVectorReader::Position::Begin);
		uint32_t Count = Reader.ReadUInt32();
		...
		Reader.Seek(CommandEnd, application::util::BinaryVectorReader::Position::Begin);
		...
		for (int i = 0; i < Header.NodeCount; i++) {
			...
			Reader.Seek(Node.ParametersOffset, application::util::BinaryVectorReader::Position::Begin);
			for (int i = 0; i < 6; i++) {
				uint32_t Index = Reader.ReadUInt32();
				uint32_t Count = Reader.ReadUInt32();
			...
			if (Counts.size() > 0) {
				for (int i = 0; i < 10; i++) {
					Reader.Seek(Start + Indices[i] * 4, application::util::BinaryVectorReader::Position::Begin);
					std::vector<uint32_t> Offsets;
			...
			Reader.Seek(NextNodeOffset, application::util::BinaryVectorReader::Position::Begin);
			this->Nodes[i] = Node;
		}
		...
		/* Embedded AINB */
		Reader.Seek(Header.EmbedAinbOffset, application::util::BinaryVectorReader::Position::Begin);
		Count = Reader.ReadUInt32();
	...
	AINBFile::AINBFile(std::string FilePath) {
		...
		{
			application::util::Logger::Error("AINBDecoder", "Could not open \"" + FilePath + "\"");
		}
	}
	...
	int GetOffsetInStringTable(std::string Value, std::vector<std::string>& StringTable)
	{
		...
		if (Index == -1)
		{
			application::util::Logger::Warning("AINBDecoder", "Could not find string \"" + Value + "\" in StringTable");
			return -1;
		}
	...
	std::vector<unsigned char> AINBFile::ToBinary()
	{
			...
			for (AINBFile::Node& Node : this->Nodes)
			{
				for (int Type = 0; Type < AINBFile::ValueTypeCount; Type++)
				{
					for (AINBFile::InputEntry& Entry : Node.InputParameters[Type])
					{
						...
						{
							if (std::find(Entry.Flags.begin(), Entry.Flags.end(), AINBFile::FlagsStruct::PulseThreadLocalStorage) == Entry.Flags.end())
								Entry.Flags.push_back(AINBFile::FlagsStruct::PulseThreadLocalStorage);
						}
					}
				}
			}
		...
		for (AINBFile::Node& Node : this->Nodes)
		{
			...
			if (!Found)
			{
				...
				Node.BasePreconditionNode = BaseIndex;
			}
			else
			{
				Node.BasePreconditionNode = Index;
			}
		}
		...
		Writer.Seek(this->Nodes.size() * 60, application::util::BinaryVectorWriter::Position::Current);
		...
		if (!GlobalParametersEmpty) {
			...
			for (int Type = 0; Type < AINBFile::GlobalTypeCount; Type++)
			{
				for (AINBFile::GlobalEntry Entry : this->GlobalParameters[Type])
				{
					...
					if (Type == (int)AINBFile::GlobalType::Vec3f)
					{
						glm::vec3* Vec3f = reinterpret_cast<glm::vec3*>(&Entry.GlobalValue);
						Writer.WriteRawUnsafeFixed(reinterpret_cast<char*>(&Vec3f->x), sizeof(float));
						Writer.WriteRawUnsafeFixed(reinterpret_cast<char*>(&Vec3f->y), sizeof(float));
						Writer.WriteRawUnsafeFixed(reinterpret_cast<char*>(&Vec3f->z), sizeof(float));
						Size += 12;
					}
			...
			Writer.Seek(Start + Size, application::util::BinaryVectorWriter::Position::Begin);
		...
		{
			Writer.Seek(48, application::util::BinaryVectorWriter::Position::Current);
		}
		...
		for (AINBFile::Node& Node : this->Nodes)
		{
			...
			for (int Type = 0; Type < AINBFile::ValueTypeCount; Type++)
			{
				...
				for (AINBFile::ImmediateParameter Entry : Node.ImmediateParameters[Type])
				{
					if (Entry.Function.InstructionCount > 0 && Entry.Function.Instructions.size() > 0)
					{
						EXBCount++;
					...
					else if (Type == (int)AINBFile::LinkedNodeMapping::StandardLink)
					{
						for (AINBFile::LinkedNodeInfo Entry : Node.LinkedNodes[Type])
						{
								...
								Node.Type == (int)AINBFile::NodeTypes::Element_RandomSelector)
							{
								...
								else if(Entry.DynamicStateName != "MapEditor_AINB_NoVal")
								{
									AddToStringTable(Entry.DynamicStateName, StringTable);
									Writer.WriteInteger(GetOffsetInStringTable(Entry.DynamicStateName, StringTable), sizeof(uint32_t));
								}
		...
		if (!this->Nodes.empty())
		{
			...
			Writer.Seek(116 + 24 * this->Commands.size(), application::util::BinaryVectorWriter::Position::Begin);
			for (AINBFile::Node Node : this->Nodes)
			{
		...
		Writer.Seek(AttachmentIndexStart, application::util::BinaryVectorWriter::Position::Begin);
		...
		if (!ImmediateParametersEmpty)
		{
			...
			for (int Type = 0; Type < AINBFile::ValueTypeCount; Type++)
			{
				for (AINBFile::ImmediateParameter Entry : ImmediateParameters[Type])
				{
					...
					{
						glm::vec3 Vec3f = *reinterpret_cast<glm::vec3*>(&Entry.Value);
						Writer.WriteRawUnsafeFixed(reinterpret_cast<char*>(&Vec3f.x), sizeof(float));
						Writer.WriteRawUnsafeFixed(reinterpret_cast<char*>(&Vec3f.y), sizeof(float));
						Writer.WriteRawUnsafeFixed(reinterpret_cast<char*>(&Vec3f.z), sizeof(float));
					}
		...
		{
			for (uint16_t PreconditionNode : PreconditionNodes)
			{
				Writer.WriteInteger(PreconditionNode, sizeof(uint16_t));
		...
		if (EXBFile.Loaded)
		{
			...
			application::util::Logger::Info("AINBEncoder", "EXB size: " + std::to_string(Bytes.size()));
		...
		for (AINBFile::Node& Node : this->Nodes)
		{
			...
			{
				if (Header.FileCategory != "Logic")
				{
					EmbeddedAINBs[Node.GetName()] = 1;
				}
				else
				{
					if (!EmbeddedAINBs.contains(Node.GetName()))
					{
						EmbeddedAINBs[Node.GetName()] = 1;
					}
					else
					{
						...
						for (uint8_t ValueType = 0; ValueType < AINBFile::ValueTypeCount; ValueType++)
						{
							for (AINBFile::ImmediateParameter& Param : Node.ImmediateParameters[ValueType])
							{
								if (Param.Name == "InstanceName")
								{
						...
						if (NeedsNewInstance)
						{
							if (Node.GetName() == "OperateORFixInputTrueWithReset.module")
								NeedsNewInstance = false;
						}

						if (NeedsNewInstance)
						{
							EmbeddedAINBs[Node.GetName()]++;
						}
					}
				}
			}
		...
		Writer.Seek(24, application::util::BinaryVectorWriter::Position::Begin);
		Writer.WriteInteger(Attachments.size(), sizeof(uint32_t));
		Writer.Seek(36, application::util::BinaryVectorWriter::Position::Begin);
