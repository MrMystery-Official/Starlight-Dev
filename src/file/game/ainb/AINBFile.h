...
//Im very sorry for that terrible code, but I am too lazy and scared to refactor it

namespace application::file::game::ainb
{
	class AINBFile {
	public:
		using AINBValue = std::variant<uint32_t, bool, float, std::string, glm::vec3>; // Same order as ValueType enum
		...
		struct Command {
			...
			bool IsLeftNodeResident = false;
			bool IsRightNodeResident = false;
		};
	...
	file_category = {"AI" : 0, "Logic" : 1, "Sequence" : 2}
		...
		*/
		...
		void Initialize(std::vector<unsigned char> Bytes, bool SkipErrors = false);
	...
	private:
		...
		AttachmentEntry ReadAttachmentEntry(application::util::BinaryVectorReader* Reader);
